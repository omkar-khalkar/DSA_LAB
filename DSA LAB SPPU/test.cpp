...
void employee::create()
{
    ...
    fstream empfile;
    fstream indexfile;
    empfile.open("emp.txt", ios::out | ios::app | ios::binary);
    indexfile.open("index.txt", ios::out | ios::app | ios::binary);
    char c = 'y';
    while (c == 'y' || c == 'Y')
    {
        cout << "\nEnter name: ";
        ...
        empfile.write((char *)&erec, sizeof(emp));
...
void employee::display()
{
    fstream empfile;
    fstream indexfile;
    empfile.open("emp.txt", ios::in | ios::binary);
    indexfile.open("index.txt", ios::in | ios::binary);

    int offset = 0;
    memset(&erec, 0, sizeof(emp)); // Clear the erec object before reading new records

    while (indexfile.read((char *)&irec, sizeof(index)))
    {
        if (irec.emp_id != -1)
        {
            offset = irec.position * sizeof(emp);
            empfile.seekg(offset, ios::beg);
            empfile.read((char *)&erec, sizeof(emp));

            cout << "\nName : " << erec.name;
            cout << "\nEmp_Id: " << erec.emp_id;
            cout << "\nSalary : " << erec.salary << endl;
        }
    }
